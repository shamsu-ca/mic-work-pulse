...
async function runRecurringAudit() {
  console.log("=== FETCHING RECURRING TEMPLATES & USER DATA ===");
  ...
  savedTasks.forEach((t, index) => {
    ...
    console.log(`Assignee ID: ${t.assignee_id || "None"}`);
    console.log(`Assignee Name: ${assigneeName}`);
    console.log(`Is Active: ${t.is_active}`);
    console.log(`Recurrence Rule: ${JSON.stringify(t.recurrence_rule)}`);
    console.log(`Last Generated At: ${t.last_generated_at || "Never"}`);
    ...
    if (t.assignee_id && !assignee) {
      console.log("    - [!] ORPHAN ASSIGNEE: The assigned user ID does not exist in the users table (Deleted User).");
    } else {
      console.log("    - [OK] Assignee is valid.");
    }

    // 2. Invalid Recurrence Rule
    let isRuleValid = true;
    if (!t.recurrence_rule || typeof t.recurrence_rule !== 'object') {
      console.log("    - [!] INVALID RECURRENCE RULE: Rule is missing or is not a valid JSON object.");
      isRuleValid = false;
    } else {
      const validTypes = ['daily', 'every_x_days', 'weekly', 'monthly', 'every_x_months', 'x_monthly'];
      if (!validTypes.includes(t.recurrence_rule.type)) {
        console.log(`    - [!] INVALID RECURRENCE RULE: Type "${t.recurrence_rule.type}" is invalid.`);
        isRuleValid = false;
      }
    }
    if (isRuleValid) {
      console.log("    - [OK] Recurrence rule structure is valid.");
    }

    // 3. Last Generated At blocking generation
    if (t.last_generated_at && t.last_generated_at >= todayStr) {
      console.log(`    - [!] LAST_GENERATED_AT BLOCK: Already generated today or in the future (${t.last_generated_at}).`);
    } else {
      console.log("    - [OK] Not yet generated today.");
    }
    ...
    if (isRuleValid && t.recurrence_rule.type === 'weekly') {
      ...
      if (rule.day !== undefined) {
        console.log(`    - Day Check: Configured to run on day ${rule.day} (Today is day ${todayDayOfWeek}).`);
      } else if (Array.isArray(rule.weekly_days)) {
        console.log(`    - Day Check: Configured to run on days [${rule.weekly_days.join(', ')}] (Today is day ${todayDayOfWeek}).`);
      }
    }

    // 5. Inactive templates
    if (!t.is_active) {
      console.log("    - [!] TEMPLATE INACTIVE: Generation is paused.");
    }

    // Trace execution for today (June 5, 2026)
    console.log("\n  * TRACE EXECUTION FOR TODAY (2026-06-05):");
    if (!t.is_active) {
      console.log("    - SKIPPED: Template is inactive.");
    } else if (t.assignee_id && !assignee) {
      console.log("    - SKIPPED: Assignee is a deleted/invalid user.");
    } else if (!isRuleValid) {
      console.log("    - SKIPPED: Recurrence rule is invalid.");
    } else {
      // Trace date loop
      ...
      if (!startDateStr) {
        if (t.recurrence_rule.type === 'daily') {
          console.log(`    - WOULD UPDATE: last_generated_at initialized to today (${todayStr}) (skipped generation for the first time).`);
          shouldCheck = false;
        } else {
      ...
      if (shouldCheck) {
        console.log(`    - Scanning dates from ${startDateStr} to ${todayStr}...`);
        ...
        if (generatedDates.length > 0) {
          console.log(`    - SUCCESS: Generated tasks on dates:`, generatedDates);
        } else {
          console.log(`    - NO GENERATION TODAY:`);
          if (skippedDates.length > 0) console.log(`      * Skipped on leaves:`, skippedDates);
          if (rescheduledDates.length > 0) console.log(`      * Rescheduled to future:`, rescheduledDates);
          console.log(`      * Scheduled match for today (${todayStr})? ${isScheduledDay(todayObj, t.recurrence_rule, t)}`);
        }
      }
    }
    console.log("\n");
